...
const AbilityPage = () => {
  ...
  // 分组 Tab：参考渠道管理「按类型」Tab。'all' = 不限分组。
  const [availableGroups, setAvailableGroups] = useState([]);
  const activeGroupKey = filterGroup === '' ? 'all' : filterGroup;
  ...
  const loadAbilities = useCallback(
    async (p, ps, model, group, channelId, keyword) => {
      setLoading(true);
      try {
        ...
        const res = await API.get(
          `/api/channel/ability/list?${params.toString()}`,
        );
  ...
  // Load available groups for top Tabs
  useEffect(() => {
    (async () => {
      try {
        ...
        const { success, data } = res.data;
        if (success && Array.isArray(data)) {
          setAvailableGroups(data.filter(Boolean).sort());
        }
      } catch {
        // ignore
      }
  ...
  const handleGroupTabChange = (key) => {
  ...
  const handleUpdateAbility = async (record, field, value) => {
    try {
      ...
      if (success) {
        ...
        setAbilities((prev) =>
          prev.map((a) =>
            a.group === record.group &&
            a.model === record.model &&
            a.channel_id === record.channel_id
              ? { ...a, [field]: value }
              : a,
          ),
        );
      ...
      },
      {
        ...
        render: (enabled, record) => (
          <Switch
            ...
            checked={enabled}
            onChange={(checked) =>
              handleUpdateAbility(record, 'enabled', checked)
            }
          />
        ),
      },
      ...
      >
          ...
          <div className='mb-4 p-4 rounded-lg bg-[var(--semi-color-fill-0)] border border-[var(--semi-color-border)]'>
            ...
            <div className='space-y-2 text-sm text-[var(--semi-color-text-1)]'>
              <div className='flex items-start gap-2'>
                <Tag color='blue' size='small' className='shrink-0 !mt-0.5'>
                  1
                </Tag>
                <span>
                  {t('系统根据请求的分组和模型，查找所有匹配的可用渠道记录')}
                </span>
              </div>
              <div className='flex items-start gap-2'>
                <Tag color='blue' size='small' className='shrink-0 !mt-0.5'>
              ...
              <div className='flex items-start gap-2'>
                ...
                <span>
                  {t('当前优先级的渠道全部失败后，自动降级到下一优先级')}
                </span>
              </div>
            </div>

            <Typography.Title heading={6} className='!mt-4 !mb-3'>
              {t('配置示例')}
            </Typography.Title>
              ...
              <table className='text-sm w-full border-collapse'>
                <thead>
                  <tr className='bg-[var(--semi-color-fill-1)]'>
                    <th className='px-3 py-1.5 text-left border border-[var(--semi-color-border)]'>
                      {t('模型')}
                    </th>
                    <th className='px-3 py-1.5 text-left border border-[var(--semi-color-border)]'>
                      {t('渠道')}
                ...
                <tbody>
                  <tr>
                    <td className='px-3 py-1.5 border border-[var(--semi-color-border)]'>
                      gpt-4
                    </td>
                    <td className='px-3 py-1.5 border border-[var(--semi-color-border)]'>
                      {t('渠道')} A
            ...
            <ul className='text-sm text-[var(--semi-color-text-2)] list-disc pl-5 space-y-1'>
              ...
              <li>
                {t('此处修改仅影响当前模型在该渠道的配比，不影响其他模型')}
              </li>
              <li>
                {t(
                  '通过渠道管理修改渠道默认优先级/权重时，仅对新增的记录生效，不会覆盖此处的独立配置',
                )}
              </li>
            </ul>
          </div>
        ...
        <Spin spinning={loading}>
          <Table
            ...
            rowKey={(record) =>
              `${record.group}|${record.model}|${record.channel_id}`
            }
            pagination={{
              currentPage: page,
              pageSize: pageSize,
