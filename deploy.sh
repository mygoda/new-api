...
generate_compose() {
  ...
  redis:
    ...
    cat > "$SCRIPT_DIR/docker-compose.deploy.yml" <<YAML
...
networks:
  app-network:
    driver: bridge
YAML
    info "生成 docker-compose.deploy.yml"
}
...
do_up() {
    ...
    if [ -n "$compose_cmd" ]; then
        ...
        $compose_cmd -f docker-compose.deploy.yml -p "$COMPOSE_PROJECT" up -d
    else
...
do_logs() {
    ...
    if [ -n "$compose_cmd" ] && [ -f "$SCRIPT_DIR/docker-compose.deploy.yml" ]; then
        $compose_cmd -f docker-compose.deploy.yml -p "$COMPOSE_PROJECT" logs -f --tail 100
    else
...
do_status() {
    ...
    if [ -n "$compose_cmd" ] && [ -f "$SCRIPT_DIR/docker-compose.deploy.yml" ]; then
        $compose_cmd -f docker-compose.deploy.yml -p "$COMPOSE_PROJECT" ps
    else
