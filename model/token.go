...
type Token struct {
	...
	RPM                int            `json:"rpm" gorm:"default:0"`  // 每分钟请求数限制，0 = 使用全局默认值
	TPM                int            `json:"tpm" gorm:"default:0"`  // 每分钟Token数限制，0 = 使用全局默认值
...
func (token *Token) Update() (err error) {
	...
	err = DB.Model(token).Select("name", "status", "expired_time", "remain_quota", "unlimited_quota",
		"model_limits_enabled", "model_limits", "allow_ips", "group", "cross_group_retry", "rpm", "tpm").Updates(token).Error
	return err
}
